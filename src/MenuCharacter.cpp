...
void MenuCharacter::refreshStats() {
	...
	if (show_stat[0]) {
		...
		statList->set(visible_stats++, ss.str(),msg->get("Each point of Physical grants +%d HP", stats->hp_per_physical, stats->hp_per_level));
	}
	...
	if (show_stat[2]) {
		...
		statList->set(visible_stats++, ss.str(),msg->get("Each point of Mental grants +%d MP", stats->mp_per_mental, stats->mp_per_level));
	}
